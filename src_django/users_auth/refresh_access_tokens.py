...
def run_refresh():
    ...
    for profile in all_profiles:
        ...
        try:
            ...
            refresh_sucesses.append(profile.user.username)
            time.sleep(2)
        except Exception as e:
            refresh_errors.append(profile.user.username)
            print(f"Error while refreshing access token: {type(e)}: {e}")
        #     raise

    return refresh_sucesses, refresh_errors

if __name__ == '__main__':
    successes, errors = run_refresh()
